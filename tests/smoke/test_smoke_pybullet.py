"""Smoke test for PyBullet backend with real rendering.

This test verifies that the PyBullet backend produces real RGB, depth, and segmentation
...
def _pybullet_available() -> bool:
    """Check if PyBullet package is available."""
    try:
...
@pytest.mark.skipif(
    ...
    reason="PyBullet package not available. Install with: pip install pybullet",
)
def test_smoke_pybullet_backend(tmp_path: Path):
    """Smoke test for PyBullet backend with real rendering.

    This test:
    1. Runs a minimal job with PyBullet backend
    ...
    config_path = (
        Path(__file__).parent.parent.parent / "examples" / "configs" / "pybullet_smoke.yaml"
    )
    ...
    config_data["out_dir"] = str(tmp_path / "smoke_pybullet")
    ...
    # Run the job
    run_job(config_path_or_obj=test_config_path, backend="pybullet")

    out_dir = tmp_path / "smoke_pybullet"
    ...
    assert manifest["jobspec"]["job_name"] == "smoke_pybullet"
    assert manifest["backend"]["name"] == "pybullet"
    ...
    package_path = tmp_path / "smoke_pybullet.zip"
...
@pytest.mark.skipif(not _pybullet_available(), reason="PyBullet package not available")
def test_smoke_pybullet_segmentation_values(tmp_path: Path):
    ...
    config_data["out_dir"] = str(tmp_path / "smoke_pybullet_seg")
    ...
    out_dir = tmp_path / "smoke_pybullet_seg"
    seg_dir = out_dir / "seg"
